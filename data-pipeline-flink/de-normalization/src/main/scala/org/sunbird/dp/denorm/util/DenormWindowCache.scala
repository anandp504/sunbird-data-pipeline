...
class DenormWindowCache(config: DenormalizationConfig, contentRedis: RedisConnect, deviceRedis: RedisConnect,
  ...
  private def getContentCacheData(contentMap: MMap[String, AnyRef], collectionMap: MMap[String, AnyRef], l2rollupMap: MMap[String, AnyRef]) = {
    contentMap.keySet.foreach {
      contentId => // contentMap.put(contentId, getDataMap(contentPipeline.get(contentId), config.contentFields))
        contentMap.put(contentId, contentPipeline.get(contentId))
    }

    collectionMap.keySet.foreach {
      collectionId => // collectionMap.put(collectionId, getDataMap(contentPipeline.get(collectionId), config.contentFields))
        collectionMap.put(collectionId, contentPipeline.get(collectionId))
    }

    l2rollupMap.keySet.foreach {
      l2RollupId => // l2rollupMap.put(l2RollupId, getDataMap(contentPipeline.get(l2RollupId), config.contentFields))
        l2rollupMap.put(l2RollupId, contentPipeline.get(l2RollupId))
    }
  }

  private def getDeviceCacheData(deviceMap: MMap[String, AnyRef]) = {
    deviceMap.keySet.foreach {
      deviceId => // convertToComplexDataTypes(getData(devicePipeline.hgetAll(deviceId), config.deviceFields))
        deviceMap.put(deviceId, devicePipeline.hgetAll(deviceId))
        // deviceMap.put(deviceId, devicePipeline.hgetAll("b93edf2bdf7aab1ebfff314cfb7f792959094ad3"))
    }
  }

  private def getUserCacheData(userMap: MMap[String, AnyRef]) = {
    userMap.keySet.foreach {
      userId => // convertToComplexDataTypes(getData(userPipeline.hgetAll(userId), config.userFields))
        userMap.put(userId, userPipeline.hgetAll(userId))
    }
  }

  private def getDialcodeCacheData(dialcodeMap: MMap[String, AnyRef]) = {
    dialcodeMap.keySet.foreach {
      dialcodeId => // dialcodeMap.put(dialcodeId, getDataMap(userPipeline.get(dialcodeId), config.dialcodeFields))
        dialcodeMap.put(dialcodeId, dialcodePipeline.get(dialcodeId))
    }
  }
