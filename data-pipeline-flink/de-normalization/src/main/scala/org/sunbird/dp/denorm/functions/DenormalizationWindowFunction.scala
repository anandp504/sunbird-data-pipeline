...
import org.sunbird.dp.core.job.{BaseProcessKeyedFunction, Metrics}
import org.sunbird.dp.denorm.`type`._
import org.sunbird.dp.denorm.domain.Event
import org.sunbird.dp.denorm.task.DenormalizationConfig
import org.sunbird.dp.denorm.util.{CacheResponseData, DenormWindowCache}
...
class DenormalizationWindowFunction(config: DenormalizationConfig)(implicit val mapTypeInfo: TypeInformation[Event])
  ...
  lazy val state: ValueState[WindowEvents] = getRuntimeContext.getState(new ValueStateDescriptor[WindowEvents]("state", classOf[WindowEvents]))
  ...
  override def processElement(event: Event,
    ...
    context.timerService().registerProcessingTimeTimer(context.timerService.currentProcessingTime() + config.windowTimeInMillis)
  ...
  def parseLookupIds(events: List[Event]): (List[EventCache], EventsMetadata) = {
    ...
    val eventsCache = events.map { event =>
      ...
      val deviceId = if (!event.did().isEmpty) Some(event.did()) else None
